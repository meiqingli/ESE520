...
[
  ...
  ;; percentage of population that has broken a traffic law (has jaywalked)
  law-breakers
  ...
  red-light_HV  ;; variable that determines the weight of perceived risk for stoping at red light (HV)
  red-light_Ped ;; variable that determines the weight of perceived risk for stoping at red light (ped)
  number_traffic_offenses ;; variable that keeps track of the number of traffic offenses committed throughout the simulation
...
[
  ...
  law-breaker? ; true if agent has ever jaywalked
...
;; Comment Below out when doing What If Scenario
    let rd random 100
    ;; risk-factor = 1
    if rd >= 0 and rd < 16 [
      set risk-factor 1
  ...
  [
    ...
    ;; Comment Below out when using What-if-Scenario
    let rd random 100
    ;; risk-factor = 1
    if rd >= 0 and rd < 16 [
      set risk-factor 1
    ...
    ;; Below is for our What-if-Scenario
    ;;set risk-factor 4
...
; calculates the percentage of law breakers in the population
to set-law-breakers
  set law-breakers 100 * (count turtles with [law-breaker? = true]) / (num-car-users + num-pedestrians)
end
...
to check-for-collisions
    ...
    [
     ...
     set number_deaths number_deaths + 2
     ]
    ]
end
...
to drive-HV
    ...
    if [pcolor = red] of patch-here [set jaywalk true set law-breaker? true set number_traffic_offenses number_traffic_offenses + 1]
    ...
    ;; make a variable called emotion_change -> every time update emotion will add emotion change as a multiplier to it -> emotion_change will be a distribution
...
to walk ;; for pedestrians
  ;; checks to see if a turtle is in intersection when light is green
    if [pcolor = green] of patch-here [set jaywalk true set law-breaker? true set number_traffic_offenses number_traffic_offenses + 1]
        ;; checks to see if turtle has past the full intersection and sets jaywalk back to false
    if [abs pxcor > 2 or abs pycor > 2] of patch-here [set jaywalk false]
    ;; if a turtle is near the intersection and there are other turtles breaking the law, emotions will go up
    if [abs pxcor <= 3 and abs pycor <= 3] of patch-here
...
to-report risk-level_HV [red-light-ahead]
  ...
  let u_risk (5 - risk-factor) * (0.12 * (countHVs + countAVs) + 0.46 * (countPeople)
    + red-light_HV * red-light-ahead) - 0.03 * emotion_level - 0.03 * foe-ahead
  report u_risk
end
...
to-report risk-level_ped [red-light-ahead]
  ...
  let u_risk (5 - risk-factor) * (0.23 * (countHVs + countAVs)
    + red-light_Ped * red-light-ahead) - 0.09 * emotion_level - 0.03 * foe-ahead
  report u_risk
end
